...
public class LaserAttackEnemy : MonoBehaviour, IEnemyCombat
{
    ...
    [Header("공격 / 대미지")]
    public EnemyLaser laserPrefab;   // 공격 레이저 프리팹
    public Transform firePoint;   // 발사 위치
    public float damage = 0.4f;  // 공격 대미지

    [Header("레이저 설정")]
    public float aimingTime = 2;    // 조준 시간
    public float timeToFire = 0.3f; // 발사까지의 시간 (조준 완료 이후)
    public float laserThickness = 0.2f; // 레이저의 두께
    public Color aimingColor = Color.red;   // 조준 색상
    public Color lockOnColor = Color.red;   // 조준 완료 색상

    [Header("발사 이후 사라짐 / 색상")]
    public float shrinkTime = 0.1f; // 발사 후 레이저가 사라지는 시간
    public Color startColor = Color.red;    // 레이저 발사 시 색상
    ...
    void Attack()
    {
        ...
        laser.damage = damage;
        laser.aimingTime = aimingTime;
        laser.timeToFire = timeToFire;
        laser.laserThickness = laserThickness;
        laser.aimingColor = aimingColor;
        laser.lockOnColor = lockOnColor;
        laser.shrinkTime = shrinkTime;
